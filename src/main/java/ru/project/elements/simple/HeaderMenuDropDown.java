...
public class HeaderMenuDropDown extends ElementPrototype {
    ...
    public void checkExist(String elementName) {
        ...
        if (!checkItemInListExist(elementName)) {
            Assert.fail("В списке нет элемента '" + elementName + "', есть только: " + getAllDropDownElementsAsString());
        }
        clickElement();
        getLifecycle().updateStep(step -> step.getParameters().remove(0));
    }
    ...
    public void checkAbsent(String elementName) {
        ...
        crutch();
        if (checkItemInListExist(elementName)) {
            Assert.fail("В списке есть элемент '" + elementName + "' (" + getAllDropDownElementsAsString() + ")");
        }
        clickElement();
        getLifecycle().updateStep(step -> step.getParameters().remove(0));
    }
    ...
    protected void selectItemFromList(String itemName) {
        ...
        if (foundItem.exists()) {
            ...
            Assert.fail("В списке нет элемента '" + itemName + "', есть только: " + getAllDropDownElementsAsString());
        }
    }
